...
int menu()  {
	...
	cout << "3. Удаление первого встречающегося элемента по городу" << endl;
...
//Удаление первого встречающегося элемента по городу
void deleteByCity(char city[CITY_SIZE], Address** phead, Address** plast) {
	struct Address* t = *phead;  //Указатель на начало списка
	while (t) {
		if (!strcmp(city, t->city)) {  //Если элемент найден, выходим из поиска
			break;
		}
		t = t->next;
	}
	if (!t) {
		cerr << "Город не найден" << endl;
	}
	else {
		if (*phead == t) {  //Если элемент находится в начале списке
			*phead = t->next;
