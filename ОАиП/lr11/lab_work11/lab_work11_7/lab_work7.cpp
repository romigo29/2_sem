...
int nLeft = 0, nRight = 0; //Подсчет количество листьев слева и справа
int getLeavesAmount(Tree* t);
...
int main() {
	...
	for (;;)
	{
		...
		cout << "7 - вывести количество листьев дерева\n";
		cout << "0 - выход\n";
		cout << "ваш выбор?\n";
		cin >> choice;
		cout << "\n";
		...
		case 7: {
			cout << "Количество листье в дереве: " << getLeavesAmount(Root) << endl;
			break;
		}
		case 0: {
			exit(0);
...
int getLeavesAmount(Tree* t) {
	...
	nLeft += getLeavesAmount(t->Left); //Просматриваем левую ветвь
	nRight += getLeavesAmount(t->Right);  //Затем правую
