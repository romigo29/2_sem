...
int main() {
	...
	for (;;)
	{
		...
		cout << "7 - найти количество вершин, значение которых равно k\n";
		cout << "0 - выход\n";
		cout << "ваш выбор?\n";
		cin >> choice;
		cout << "\n";
		...
		case 7: {
			...
			cin >> k;
			cout << "количество вершин, значение которых равно k: " << getCoincidences(Root, k, amount) << endl;
			break;
		}
		case 0: {
			exit(0);
...
int getCoincidences(Tree* t, int k, int amount) {
	...
	if (t->key == k) {
		amount++;  // Увеличиваем счетчик, если ключ текущей вершины равен k
	}

	// Рекурсивно обходим левое и правое поддеревья, суммируя количество совпадений
